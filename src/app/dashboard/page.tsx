  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        const currentUserEntry = matrixData.matrix.find(
          (user: any) =>
            user.name === "You" ||
            user.scores.some((score: any) => score.score === 100)
        );
        ...
        if (currentUserEntry) {
          currentUserEntry.scores.forEach((connection: any) => {
            if (connection.dimension_scores && connection.score !== 100) {
              // exclude self
        ...
        const dimensionBreakdown = Object.entries(allDimensionScores).map(
          ([id, data]: [string, any]) => {
    ...
    if (assessmentsData && !assessmentsLoading) {
      ...
      const formattedAssessments = assessments
        ...
        .map((assessment: any) => ({
          ...
          completedAt:
            assessment.status === "completed" ? assessment.updated_at : null,
        }))
        .sort((a: any, b: any) => {
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your highest compatibility matches
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-4">
                  {topConnections.map((connection) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
                          <span className="text-xs font-medium">
                            ...
                            {connection.name.split(" ").length > 1
                      ...
                      <Badge
                        variant={
                          connection.score > 90 ? "default" : "secondary"
                        }
                      >
                        {connection.score}%
                      </Badge>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your compatibility across different dimensions
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Suggested actions based on your profile
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                {(!assessmentsData ||
                  assessmentsData.overall_progress < 100) && (
                  ...
                  (assessmentsData.overall_progress < 100 && (
                    <div className="flex gap-4 p-4 rounded-lg border bg-muted/30">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Enhance your compatibility matching with objective
                          biometric data.
                        </p>
                        <Button variant="link" className="h-8 p-0 mt-2" asChild>
                          <Link href="/biometrics">Measure HRV →</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
                ...
                {topConnections.length > 0 && (
                  <div className="flex gap-4 p-4 rounded-lg border bg-muted/30">
                    ...
                    <div>
                      <h3 className="font-medium">
                        Connect with {topConnections[0].name}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Your compatibility score of {topConnections[0].score}%
                        suggests a strong potential connection.
                      </p>
                      <Button variant="link" className="h-8 p-0 mt-2" asChild>
                        <Link
                          href={`/compatibility/${topConnections[0].user_id}`}
                        >
                          View Compatibility →
                        </Link>
                      </Button>
                ...
                <div className="flex gap-4 p-4 rounded-lg border bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Adding more details to your profile will improve
            ...
            <CardFooter>
              <Button variant="outline" className="w-full" asChild>
                <Link href="/connections/suggested">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {topConnections.slice(0, 4).map((connection) => (
                <CompatibilityCard
                  ...
                  dimensions={connection.dimension_scores
                    ?.slice(0, 3)
                    .map((dim: any) => ({
                      name: dim.name || dim.dimension_id,
                      score: dim.score,
                    }))}
                  strengths={connection.strengths?.map(
                    (s: any) =>
                      s.description || `Strong ${s.name || s.dimension_id}`
                  )}
                  challenges={connection.challenges?.map(
                    (c: any) =>
